...
def gen(seed: t.Optional[str] = None) -> t.Any:
    ...
    try:
        r: flask.Response = flask.Response(
...
{sp.check_output(("trie-generate", "model.bin", seed, str(flask.request.args.get("min", 16)), str(flask.request.args.get("count", 8)))).decode("utf8").strip()}
----- END LIBTRIE GENERATED TEXT -----""",
            mimetype="text/plain",
        )
    except Exception:
        return flask.Response("Bad request.", mimetype="text/plain"), 400
